...
class Utils {
  ...
  /**
   * Configures paths to the GStreamer libraries. On Windows queries various GStreamer environment variables, and then
   * sets up the PATH environment variable. On macOS, adds the location to jna.library.path (macOS binaries link to each
   * other). On both, the gstreamer.path system property can be used to override. On Linux, assumes GStreamer is in the
   * path already.
   */
  ...
  /**
   * Query over a stream of possible environment variables for GStreamer location, filtering on the first non-null
   * result, and adding \bin\ to the value.
  ...
  static String findWindowsLocation() {
    if(Platform.is64Bit()) {
      return Stream.of("GSTREAMER_1_0_ROOT_MSVC_X86_64", "GSTREAMER_1_0_ROOT_MINGW_X86_64", "GSTREAMER_1_0_ROOT_X86_64")
        .map(System::getenv).filter(p -> p != null).map(p -> p.endsWith("\\") ? p + "bin\\" : p + "\\bin\\").findFirst()
        .orElse("");
    } else {
      return "";
    }
